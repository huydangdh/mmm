        ...
        console.log(
            `${new Date()
                ...
                .slice(
                    0,
                    -3
                )}=TCL: handleDeviceConnected -> ${deviceName},${data}`
        );
    }
    handleDeviceError(deviceName: string, error: any) {
        console.log(
            `${new Date()
                .toISOString()
                .replace(/[^0-9]/g, "")
                .slice(0, -3)}=TCL: handleDeviceError -> ${deviceName},${error}`
        );
    }
    handleDeviceClose(deviceName: string, error: any) {
        console.log(
            `${new Date()
                .toISOString()
                .replace(/[^0-9]/g, "")
                .slice(0, -3)}=TCL: handleDeviceClose -> ${deviceName},${error}`
        );
    }
    ...
    private async handleDataFromDevice(deviceID: number, data: string) {
        ...
        callProcessDeviceData(deviceID, data).then(res => {
            ...
            console.log(
                `${new Date()
                    .toISOString()
                    .replace(/[^0-9]/g, "")
                    .slice(0, -3)}=DB_res_${deviceID}: ${data}`
            );
        });
        ...
        console.log(
            `${new Date()
                .toISOString()
                .replace(/[^0-9]/g, "")
                .slice(0, -3)}=Data received from ${deviceID}: ${data}`
        );
    }
